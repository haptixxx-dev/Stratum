#!/bin/bash
# Generate C4 Architecture PDF with Table of Contents
#
# Dependencies:
#   - plantuml: brew install plantuml (or apt install plantuml)
#   - pandoc: brew install pandoc (or apt install pandoc)
#   - texlive: brew install --cask mactex (or apt install texlive-latex-extra)
#
# Alternative (simpler, no LaTeX needed):
#   - pandoc with wkhtmltopdf: brew install wkhtmltopdf

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

echo "=== Stratum C4 Architecture PDF Generator ==="

# Create output directories
mkdir -p images/c4
mkdir -p build

echo "[1/3] Generating PlantUML diagrams..."

# Generate all diagrams as PNG (high resolution)
plantuml -tpng -Sdpi=150 c4-architecture.puml -o images/c4/

# Also generate SVG for better quality (optional)
# plantuml -tsvg c4-architecture.puml -o images/c4/

echo "[2/3] Creating combined markdown document..."

# Create the combined document with embedded images
cat > build/c4-architecture-full.md << 'EOF'
---
title: "Stratum C4 Architecture"
subtitle: "OpenStreetMap to 3D City Generation"
author: "Stratum Project"
date: \today
toc: true
toc-depth: 3
geometry: margin=1in
header-includes:
  - \usepackage{float}
  - \floatplacement{figure}{H}
---

\newpage

# Introduction

This document describes the architecture of **Stratum**, an application for converting OpenStreetMap data into optimized, textured 3D maps with procedural generation capabilities.

The architecture is documented using the [C4 model](https://c4model.com/), which provides four levels of abstraction:

1. **System Context** - How Stratum fits into the world
2. **Container** - High-level building blocks
3. **Component** - Components within containers
4. **Code** - Class-level detail (optional)

\newpage

# Level 1: System Context

Shows Stratum in the context of its users and external systems.

## Users

| Actor | Description |
|-------|-------------|
| **Developer** | Creates and edits 3D city maps using the Editor GUI |
| **Game Developer** | Integrates the Core Library into game engines |

## External Systems

| System | Interaction |
|--------|-------------|
| **OpenStreetMap** | Source of geographic data (.osm/.pbf files) |
| **Game Engine (X3)** | Target for exported 3D city data |
| **DCC Tools** | 3D software (Blender, Maya) for further asset editing |

![System Context Diagram](images/c4/C4_SystemContext.png)

\newpage

# Level 2: Container Diagram

Shows the high-level building blocks of Stratum.

## Containers

| Container | Technology | Purpose |
|-----------|------------|---------|
| **Stratum Editor** | C++20, SDL3, ImGui | Visualization and editing tool |
| **Stratum Core Library** | C++20, Static Library | Pure logic library (no graphics dependencies) |
| **Asset Cache** | File System | Cached tiles, textures, processed data |
| **Project Files** | File System | User projects and settings |

## Key Architectural Decisions

1. **Library-First Design**: Core Library has no graphics API dependencies, enabling integration into any game engine.
2. **Editor as Debug Tool**: The Editor is a thin visualization wrapper around the Core Library.
3. **Tile-Based Loading**: Geographic data is managed in tiles for efficient memory use.

![Container Diagram](images/c4/C4_Container.png)

\newpage

# Level 3: Component Diagrams

## Core Library Components

The Core Library contains the pure logic for OSM processing, procedural generation, and scene management.

| Component | Technology | Responsibility |
|-----------|------------|----------------|
| **Scene Module** | EnTT, GLM | ECS-based scene graph and spatial indexing |
| **OSM Module** | libosmium, Clipper2 | Parsing and geographic feature extraction |
| **ProcGen Module** | C++20 | Procedural generation algorithms |
| **Mesh Builder** | earcut, meshoptimizer | 3D mesh generation from 2D data |
| **Materials Module** | MaterialX | Material definitions and shader parameters |
| **Export Module** | Assimp | Format conversion (glTF, FBX, OBJ) |

![Core Library Components](images/c4/C4_Component_CoreLib.png)

\newpage

## Editor Components

The Editor provides visualization and interaction with the Core Library.

| Component | Technology | Responsibility |
|-----------|------------|----------------|
| **Application Core** | SDL3 | Window, events, main loop |
| **Renderer** | SDL_GPU | GPU resource management and rendering |
| **Camera System** | GLM | View matrices and frustum culling |
| **Debug Visualization** | Im3D | Immediate-mode debug rendering |
| **UI Panels** | Dear ImGui | Editor interface panels |
| **Gizmos** | ImGuizmo | 3D manipulation tools |

![Editor Components](images/c4/C4_Component_Editor.png)

\newpage

## Scene Module Components

The Scene Module manages all entities using an Entity Component System.

| Component | Responsibility |
|-----------|----------------|
| **Entity Registry** | Central ECS storage (EnTT) |
| **Spatial Index** | BVH for efficient spatial queries |
| **Transform System** | Position, rotation, scale management |
| **Component Types** | Data structures for entities |
| **Query System** | Efficient entity filtering |

![Scene Module Components](images/c4/C4_Component_Scene.png)

\newpage

## OSM Module Components

The OSM Module handles parsing and feature extraction from OpenStreetMap data.

| Component | Responsibility |
|-----------|----------------|
| **OSM Parser** | .osm/.pbf file parsing |
| **Coordinate System** | Lat/lon to Cartesian conversion |
| **Feature Classifier** | Tag-based feature categorization |
| **Tile Manager** | Tile loading/unloading, LOD |
| **Road Network Builder** | Road graph construction |
| **Building Extractor** | Footprint extraction |

![OSM Module Components](images/c4/C4_Component_OSM.png)

\newpage

## ProcGen Module Components

The ProcGen Module contains algorithms for procedural city generation.

| Component | Responsibility |
|-----------|----------------|
| **Terrain Generator** | Terrain mesh and elevation |
| **Road Network Generator** | Road segments with geometry |
| **Zoning System** | Zone type assignment |
| **Lot Subdivision** | Block to parcel splitting |
| **Building Generator** | 3D building volumes |

![ProcGen Module Components](images/c4/C4_Component_ProcGen.png)

\newpage

## Renderer Module Components

The Renderer Module handles all GPU interactions via SDL3's GPU API.

| Component | Responsibility |
|-----------|----------------|
| **GPU Device** | SDL_GPU context management |
| **Pipeline Manager** | Shader/pipeline caching |
| **Mesh Renderer** | Buffer management, draw calls |
| **Texture Manager** | GPU texture handling |
| **Camera** | View/projection matrices |
| **Culling System** | Visibility determination |

![Renderer Module Components](images/c4/C4_Component_Renderer.png)

\newpage

# Dynamic Diagrams

## Application Lifecycle

Shows the startup, main loop, and shutdown sequence.

1. `main()` creates Application
2. Application initializes Window (SDL3)
3. Application initializes ImGui context
4. Application initializes Renderer (GPU device)
5. Application enters main loop
6. Loop: Process events → Update → Render
7. Shutdown in reverse order

![Application Lifecycle](images/c4/C4_Dynamic_AppLifecycle.png)

\newpage

## OSM Import Flow

Shows how OSM data flows through the system during import.

1. User selects .osm/.pbf file in OSM Panel
2. Panel calls `parser.parse_file(path)`
3. Parser extracts nodes, ways, relations
4. Features classified by OSM tags
5. Mesh Builder generates geometry
6. Scene populated with entities
7. Import complete notification

![OSM Import Flow](images/c4/C4_Dynamic_OSMImport.png)

\newpage

## Frame Rendering Flow

Shows the per-frame rendering pipeline.

1. Poll SDL events
2. Forward events to ImGui
3. Update scene systems
4. Frustum cull visible entities
5. Submit draw calls to renderer
6. Sort by material/depth
7. Execute render pass
8. Render UI overlay
9. Present frame to display

![Frame Rendering Flow](images/c4/C4_Dynamic_FrameRender.png)

\newpage

# Deployment Diagram

Shows how Stratum is deployed on a user's machine.

![Deployment Diagram](images/c4/C4_Deployment.png)

## Deployment Structure

```
User's Machine
├── Operating System (Windows/macOS/Linux)
│   ├── GPU Driver (Vulkan/Metal/D3D12)
│   │   └── GPU
│   └── File System
│       ├── Projects (user files)
│       ├── Cache (tiles, textures)
│       └── Settings (preferences)
└── Application Directory
    ├── stratum (executable)
    ├── SDL3 (shared library)
    └── Assets/
        ├── Shaders
        ├── Default Materials
        └── UI Assets
```

\newpage

# Technology Stack

| Layer | Technology | Purpose |
|-------|------------|---------|
| **Windowing** | SDL3 | Cross-platform window/input/GPU |
| **UI** | Dear ImGui | Immediate-mode editor UI |
| **Rendering** | SDL_GPU | Cross-platform GPU abstraction |
| **ECS** | EnTT | Entity Component System |
| **Math** | GLM | Linear algebra |
| **OSM Parsing** | libosmium | Geographic data parsing |
| **Geometry** | Clipper2, earcut | Polygon ops, triangulation |
| **Optimization** | meshoptimizer | LOD, mesh compression |
| **Materials** | MaterialX | Material definitions |
| **Export** | Assimp | 3D format conversion |
| **Profiling** | Tracy | Performance analysis |
| **Scripting** | pybind11 | Python bindings |

\newpage

# Appendix: Component Relationships

```
┌─────────────────────────────────────────────────────────────┐
│                     STRATUM EDITOR                          │
│  ┌─────────┐  ┌──────────┐  ┌─────────┐  ┌──────────────┐  │
│  │ App     │  │ Renderer │  │ Camera  │  │  UI Panels   │  │
│  │ Core    │──│ (SDL_GPU)│──│ System  │──│  (ImGui)     │  │
│  └────┬────┘  └────┬─────┘  └─────────┘  └──────┬───────┘  │
│       │            │                            │          │
└───────┼────────────┼────────────────────────────┼──────────┘
        │            │                            │
        ▼            ▼                            ▼
┌─────────────────────────────────────────────────────────────┐
│                 STRATUM CORE LIBRARY                        │
│  ┌─────────┐  ┌──────────┐  ┌─────────┐  ┌──────────────┐  │
│  │ Scene   │◄─│ OSM      │  │ ProcGen │  │  Materials   │  │
│  │ Module  │  │ Module   │──│ Module  │  │  Module      │  │
│  └────┬────┘  └──────────┘  └─────────┘  └──────────────┘  │
│       │                                                     │
│       ▼                                                     │
│  ┌─────────┐  ┌──────────────┐                             │
│  │ Mesh    │  │   Export     │                             │
│  │ Builder │──│   Module     │                             │
│  └─────────┘  └──────────────┘                             │
└─────────────────────────────────────────────────────────────┘
```
EOF

echo "[3/3] Generating PDF..."

# Method 1: Using pandoc with XeLaTeX (best quality, Unicode support)
if command -v xelatex &> /dev/null; then
    pandoc build/c4-architecture-full.md \
        -o build/stratum-c4-architecture.pdf \
        --pdf-engine=xelatex \
        --toc \
        --toc-depth=3 \
        -V colorlinks=true \
        -V linkcolor=blue \
        -V urlcolor=blue \
        -V toccolor=gray \
        -V monofont="FiraCode Nerd Font Mono"
    echo "✓ PDF generated: build/stratum-c4-architecture.pdf"
elif command -v pdflatex &> /dev/null; then
    # Fallback to pdflatex (may fail on Unicode)
    pandoc build/c4-architecture-full.md \
        -o build/stratum-c4-architecture.pdf \
        --pdf-engine=pdflatex \
        --toc \
        --toc-depth=3 \
        -V colorlinks=true \
        -V linkcolor=blue \
        -V urlcolor=blue \
        -V toccolor=gray
    echo "✓ PDF generated: build/stratum-c4-architecture.pdf"
# Method 2: Using pandoc with wkhtmltopdf (simpler, no LaTeX)
elif command -v wkhtmltopdf &> /dev/null; then
    pandoc build/c4-architecture-full.md \
        -o build/stratum-c4-architecture.pdf \
        --pdf-engine=wkhtmltopdf \
        --toc \
        --toc-depth=3
    echo "✓ PDF generated: build/stratum-c4-architecture.pdf"
# Method 3: Generate HTML if no PDF engine available
else
    pandoc build/c4-architecture-full.md \
        -o build/stratum-c4-architecture.html \
        --standalone \
        --toc \
        --toc-depth=3 \
        --metadata title="Stratum C4 Architecture"
    echo "✓ HTML generated: build/stratum-c4-architecture.html"
    echo "  (Install texlive or wkhtmltopdf for PDF output)"
fi

echo ""
echo "=== Done ==="
echo "Output location: docs/build/"
