 ...
 *
 * Mirrors the per-slot table in the material_variant namespace, which in turn
 * mirrors osm/road/road_style.hpp.
 *
 * Slot-aware, because the vocabulary is per slot: variant 1 is worn asphalt, a
 * granite kerb, or paving-stone footway depending on which slot is asking. An
 * earlier version took the number alone, which was only ever correct while the two
 * sides shared one global numbering -- and they did not agree even then.
 *
 * A number this does not name is shown as its digits, which is what someone
 * reconciling a new variant against the geometry side needs to see.
 */
const char* variant_name(MaterialId slot, uint16_t variant) {
    ...
    switch (slot) {
        case MaterialId::Asphalt:
            switch (variant) {
                case kAsphaltWorn:      return "worn";
                case kAsphaltSmooth:    return "smooth";
                case kAsphaltCobble:    return "cobblestone";
                case kAsphaltSett:      return "sett";
                case kAsphaltPaving:    return "paving stones";
                case kAsphaltChipseal:  return "chipseal";
                case kAsphaltColoured:  return "coloured";
                default: return nullptr;
            }
        case MaterialId::Concrete:
            switch (variant) {
                case kConcreteWorn:   return "worn";
                case kConcreteSmooth: return "smooth";
                case kConcretePlates: return "plates";
                case kConcreteLanes:  return "wheel lanes";
                case kConcreteIsland: return "island";
                default: return nullptr;
            }
        case MaterialId::Curb:
            switch (variant) {
                case kCurbGranite:   return "granite";
                case kCurbAsphalt:   return "rolled asphalt";
                case kCurbDropped:   return "dropped";
                case kCurbMountable: return "mountable";
                default: return nullptr;
            }
        case MaterialId::Sidewalk:
            switch (variant) {
                case kSidewalkPaved:    return "paving stones";
                case kSidewalkTactile:  return "tactile";
                case kSidewalkAsphalt:  return "asphalt";
                case kSidewalkConcrete: return "concrete";
                case kSidewalkBrick:    return "brick";
                default: return nullptr;
            }
        case MaterialId::Markings:
            switch (variant) {
                case kMarkingsYellow: return "yellow";
                case kMarkingsWorn:   return "worn";
                default: return nullptr;
            }
        case MaterialId::Gravel:
            switch (variant) {
                case kGravelCompacted: return "compacted";
                case kGravelFine:      return "fine";
                case kGravelPebble:    return "pebble";
                case kGravelUnpaved:   return "unpaved";
                default: return nullptr;
            }
        case MaterialId::Dirt:
            switch (variant) {
                case kDirtGround: return "ground";
                case kDirtEarth:  return "earth";
                case kDirtMud:    return "mud";
                case kDirtSand:   return "sand";
                default: return nullptr;
            }
        case MaterialId::Grass:
            switch (variant) {
                case kGrassMown:    return "mown";
                case kGrassRough:   return "rough";
                case kGrassPaver:   return "grass paver";
                case kGrassPlanted: return "planted";
                default: return nullptr;
            }
        case MaterialId::BridgeDeck:
            switch (variant) {
                case kBridgeDeckSteel: return "steel";
                case kBridgeDeckWood:  return "timber";
                case kBridgeDeckStone: return "masonry";
                default: return nullptr;
            }
        case MaterialId::Parapet:
            switch (variant) {
                case kParapetSteel:    return "steel";
                case kParapetStone:    return "masonry";
                case kParapetConcrete: return "concrete";
                default: return nullptr;
            }
        case MaterialId::Wall:
            switch (variant) {
                case kWallBrick:    return "brick";
                case kWallStone:    return "stone";
                case kWallConcrete: return "concrete";
                case kWallRender:   return "render";
                case kWallGlass:    return "glass";
                case kWallMetal:    return "metal";
                case kWallWood:     return "timber";
                default: return nullptr;
            }
        case MaterialId::Roof:
            switch (variant) {
                case kRoofTile:     return "tile";
                case kRoofSlate:    return "slate";
                case kRoofMetal:    return "metal";
                case kRoofMembrane: return "membrane";
                case kRoofConcrete: return "concrete";
                case kRoofThatch:   return "thatch";
                case kRoofGlass:    return "glass";
                default: return nullptr;
            }
        default:
            return nullptr;
...
void format_key(char* out, size_t n, MaterialKey key) {
    ...
    if (const char* v = variant_name(key.material, key.variant)) {
    ...
    for (uint8_t slot_index = 0; slot_index < static_cast<uint8_t>(MaterialId::Count);
        ...
        if (ImGui::TreeNodeEx(material_id_name(slot), flags, "%s (%zu)",
            ...
            for (size_t i = first; i < cursor; ++i) {
                ...
                const char* label = variant_name(key.material, key.variant);
