...
/**
 * Variant numbers, MIRRORED from osm/road/road_style.hpp.
 *
 * That header is the authority: the geometry side derives these from OSM tags, so
 * it decides what a number means and this side must agree. They are mirrored rather
 * than included because the renderer has no business depending on the OSM layer --
 * MaterialKey was deliberately put in mesh.hpp so neither owns the other.
 *
 * Drift between the two is a silent failure: a road tagged smoothness=bad renders as
 * freshly laid, which looks entirely plausible and is entirely wrong. So
 * tests/renderer/test_variant_agreement.cpp cross-checks every constant here against
 * its road_style.hpp counterpart, and that test is the reason this mirror is safe.
 *
 * Numbering is PER SLOT. Variant 1 of Asphalt and variant 1 of Curb are unrelated,
 * exactly as SubMesh::variant documents. An earlier draft of this file used one
 * global vocabulary; it disagreed with the geometry side on the worn/smooth axis
 * (1 and 2 were swapped) and every worn road would have drawn as new.
 */

// MaterialId::Asphalt -- the bound running surface. Cobbles and setts live here
// because they are what a vehicle drives on, not because they are bitumen.
inline constexpr uint16_t kAsphaltDefault   = 0;
inline constexpr uint16_t kAsphaltWorn      = 1;
inline constexpr uint16_t kAsphaltSmooth    = 2;
inline constexpr uint16_t kAsphaltCobble    = 3;
inline constexpr uint16_t kAsphaltSett      = 4;
inline constexpr uint16_t kAsphaltPaving    = 5;
inline constexpr uint16_t kAsphaltChipseal  = 6;
inline constexpr uint16_t kAsphaltColoured  = 7;

// MaterialId::Concrete -- rigid pavement, junction slabs, hard standing.
inline constexpr uint16_t kConcreteDefault  = 0;
inline constexpr uint16_t kConcreteWorn     = 1;
inline constexpr uint16_t kConcreteSmooth   = 2;
inline constexpr uint16_t kConcretePlates   = 3;
inline constexpr uint16_t kConcreteLanes    = 4;
inline constexpr uint16_t kConcreteIsland   = 5;

// MaterialId::Curb -- face and top share a slot and therefore a texture.
inline constexpr uint16_t kCurbDefault      = 0;
inline constexpr uint16_t kCurbGranite      = 1;
inline constexpr uint16_t kCurbAsphalt      = 2;
inline constexpr uint16_t kCurbDropped      = 3;
inline constexpr uint16_t kCurbMountable    = 4;

// MaterialId::Sidewalk -- footway surface.
inline constexpr uint16_t kSidewalkDefault  = 0;
inline constexpr uint16_t kSidewalkPaved    = 1;
inline constexpr uint16_t kSidewalkTactile  = 2;
inline constexpr uint16_t kSidewalkAsphalt  = 3;
inline constexpr uint16_t kSidewalkConcrete = 4;
inline constexpr uint16_t kSidewalkBrick    = 5;

// MaterialId::Markings.
inline constexpr uint16_t kMarkingsDefault  = 0;
inline constexpr uint16_t kMarkingsYellow   = 1;
inline constexpr uint16_t kMarkingsWorn     = 2;

// MaterialId::Gravel.
inline constexpr uint16_t kGravelDefault    = 0;
inline constexpr uint16_t kGravelCompacted  = 1;
inline constexpr uint16_t kGravelFine       = 2;
inline constexpr uint16_t kGravelPebble     = 3;
inline constexpr uint16_t kGravelUnpaved    = 4;

// MaterialId::Dirt.
inline constexpr uint16_t kDirtDefault      = 0;
inline constexpr uint16_t kDirtGround       = 1;
inline constexpr uint16_t kDirtEarth        = 2;
inline constexpr uint16_t kDirtMud          = 3;
inline constexpr uint16_t kDirtSand         = 4;

// MaterialId::Grass.
inline constexpr uint16_t kGrassDefault     = 0;
inline constexpr uint16_t kGrassMown        = 1;
inline constexpr uint16_t kGrassRough       = 2;
inline constexpr uint16_t kGrassPaver       = 3;
inline constexpr uint16_t kGrassPlanted     = 4;

// MaterialId::BridgeDeck.
inline constexpr uint16_t kBridgeDeckDefault = 0;
inline constexpr uint16_t kBridgeDeckSteel   = 1;
inline constexpr uint16_t kBridgeDeckWood    = 2;
inline constexpr uint16_t kBridgeDeckStone   = 3;

// MaterialId::Parapet.
inline constexpr uint16_t kParapetDefault   = 0;
inline constexpr uint16_t kParapetSteel     = 1;
inline constexpr uint16_t kParapetStone     = 2;
inline constexpr uint16_t kParapetConcrete  = 3;

// MaterialId::Wall -- building facade.
inline constexpr uint16_t kWallDefault      = 0;
inline constexpr uint16_t kWallBrick        = 1;
inline constexpr uint16_t kWallStone        = 2;
inline constexpr uint16_t kWallConcrete     = 3;
inline constexpr uint16_t kWallRender       = 4;
inline constexpr uint16_t kWallGlass        = 5;
inline constexpr uint16_t kWallMetal        = 6;
inline constexpr uint16_t kWallWood         = 7;

// MaterialId::Roof.
inline constexpr uint16_t kRoofDefault      = 0;
inline constexpr uint16_t kRoofTile         = 1;
inline constexpr uint16_t kRoofSlate        = 2;
inline constexpr uint16_t kRoofMetal        = 3;
inline constexpr uint16_t kRoofMembrane     = 4;
inline constexpr uint16_t kRoofConcrete     = 5;
inline constexpr uint16_t kRoofThatch       = 6;
inline constexpr uint16_t kRoofGlass        = 7;

/// Highest variant number any slot uses, plus one. Sizing only; the vocabulary is
/// per slot, so this is not "the number of variants of a given material".
inline constexpr uint16_t kMaxVariantsPerSlot = 8;
