...
glm::vec2 material_tile_metres(MaterialId material) {
    switch (material) {
        ...
        case MaterialId::Wall:       return { 4.0f, 4.0f };   // facade, brick/stone scale
        case MaterialId::Roof:       return { 2.0f, 2.0f };   // tile/slate courses are small
...
void MaterialLibrary::load_defaults() {
    ...
    // Wall and Roof were added to MaterialId by the road-optimization branch for
    // building_wall_material()/building_roof_material() in osm/road/road_style.hpp.
    // Nothing emits either slot yet, but load_defaults() promises a definition for
    // EVERY slot, and a slot without one resolves to Default -- a grey building.
    surface(MaterialId::Wall,       0, "Wall",       { 0.66f, 0.63f, 0.58f }, 0.84f);
    surface(MaterialId::Roof,       0, "Roof",       { 0.34f, 0.30f, 0.28f }, 0.86f);
...
void MaterialLibrary::load_variant_defaults() {
    ...
    // --- variants ------------------------------------------------------------
    // Numbering mirrors osm/road/road_style.hpp, which is the authority because the
    // geometry side derives these from OSM tags. tests/renderer/test_variant_agreement
    // cross-checks the two so they cannot drift; a mismatch on the worn/smooth axis
    // in particular is silent, because a worn road drawn as freshly laid still looks
    // like a road.
    ...
    surface(MaterialId::Asphalt, kAsphaltWorn,     "Asphalt/worn",          { 0.17f, 0.165f, 0.16f }, 0.90f);
    surface(MaterialId::Asphalt, kAsphaltSmooth,   "Asphalt/smooth",        { 0.13f, 0.13f, 0.14f }, 0.68f);
    surface(MaterialId::Asphalt, kAsphaltCobble,   "Asphalt/cobblestone",   { 0.30f, 0.28f, 0.26f }, 0.80f);
    surface(MaterialId::Asphalt, kAsphaltSett,     "Asphalt/sett",          { 0.26f, 0.25f, 0.24f }, 0.72f);
    surface(MaterialId::Asphalt, kAsphaltPaving,   "Asphalt/paving-stones", { 0.42f, 0.40f, 0.38f }, 0.78f);
    surface(MaterialId::Asphalt, kAsphaltChipseal, "Asphalt/chipseal",      { 0.22f, 0.21f, 0.20f }, 0.92f);
    surface(MaterialId::Asphalt, kAsphaltColoured, "Asphalt/coloured",      { 0.32f, 0.14f, 0.12f }, 0.74f);

    // Rigid pavement. Cobbles have no meaning here; those numbers are plates/lanes.
    surface(MaterialId::Concrete, kConcreteWorn,   "Concrete/worn",   { 0.44f, 0.43f, 0.41f }, 0.88f);
    surface(MaterialId::Concrete, kConcreteSmooth, "Concrete/smooth", { 0.56f, 0.55f, 0.53f }, 0.62f);
    surface(MaterialId::Concrete, kConcretePlates, "Concrete/plates", { 0.52f, 0.51f, 0.49f }, 0.72f);
    surface(MaterialId::Concrete, kConcreteLanes,  "Concrete/lanes",  { 0.50f, 0.49f, 0.47f }, 0.76f);
    surface(MaterialId::Concrete, kConcreteIsland, "Concrete/island", { 0.58f, 0.57f, 0.55f }, 0.70f);

    // Footways. Sidewalk/asphalt is a bitumen footway and is dark; that is not a
    // copy-paste from the Asphalt slot.
    surface(MaterialId::Sidewalk, kSidewalkPaved,    "Sidewalk/paving-stones", { 0.58f, 0.57f, 0.55f }, 0.76f);
    surface(MaterialId::Sidewalk, kSidewalkTactile,  "Sidewalk/tactile",       { 0.62f, 0.55f, 0.30f }, 0.72f);
    surface(MaterialId::Sidewalk, kSidewalkAsphalt,  "Sidewalk/asphalt",       { 0.22f, 0.22f, 0.23f }, 0.78f);
    surface(MaterialId::Sidewalk, kSidewalkConcrete, "Sidewalk/concrete",      { 0.46f, 0.45f, 0.43f }, 0.90f);
    surface(MaterialId::Sidewalk, kSidewalkBrick,    "Sidewalk/brick",         { 0.34f, 0.32f, 0.30f }, 0.82f);

    // Kerbs. Granite is darker and markedly glossier than a precast concrete kerb,
    // which is the difference someone is looking for when they tag it.
    surface(MaterialId::Curb, kCurbGranite,   "Curb/granite",   { 0.42f, 0.42f, 0.44f }, 0.45f);
    surface(MaterialId::Curb, kCurbAsphalt,   "Curb/asphalt",   { 0.20f, 0.20f, 0.21f }, 0.88f);
    surface(MaterialId::Curb, kCurbDropped,   "Curb/dropped",   { 0.55f, 0.54f, 0.51f }, 0.82f);
    surface(MaterialId::Curb, kCurbMountable, "Curb/mountable", { 0.53f, 0.52f, 0.50f }, 0.80f);

    // Unpaved running surfaces.
    surface(MaterialId::Gravel, kGravelCompacted, "Gravel/compacted", { 0.44f, 0.42f, 0.38f }, 0.88f);
    surface(MaterialId::Gravel, kGravelFine,      "Gravel/fine",      { 0.50f, 0.48f, 0.44f }, 0.90f);
    surface(MaterialId::Gravel, kGravelPebble,    "Gravel/pebble",    { 0.48f, 0.46f, 0.43f }, 0.86f);
    surface(MaterialId::Gravel, kGravelUnpaved,   "Gravel/unpaved",   { 0.42f, 0.40f, 0.36f }, 0.92f);

    surface(MaterialId::Dirt, kDirtGround, "Dirt/ground", { 0.32f, 0.27f, 0.20f }, 0.94f);
    surface(MaterialId::Dirt, kDirtEarth,  "Dirt/earth",  { 0.28f, 0.22f, 0.16f }, 0.95f);
    surface(MaterialId::Dirt, kDirtMud,    "Dirt/mud",    { 0.20f, 0.16f, 0.12f }, 0.70f);
    surface(MaterialId::Dirt, kDirtSand,   "Dirt/sand",   { 0.62f, 0.56f, 0.42f }, 0.92f);

    surface(MaterialId::Grass, kGrassMown,    "Grass/mown",    { 0.20f, 0.34f, 0.14f }, 0.90f);
    surface(MaterialId::Grass, kGrassRough,   "Grass/rough",   { 0.26f, 0.32f, 0.16f }, 0.94f);
    surface(MaterialId::Grass, kGrassPaver,   "Grass/paver",   { 0.34f, 0.38f, 0.26f }, 0.88f);
    surface(MaterialId::Grass, kGrassPlanted, "Grass/planted", { 0.18f, 0.28f, 0.14f }, 0.92f);

    // Bridge structure.
    surface(MaterialId::BridgeDeck, kBridgeDeckSteel, "BridgeDeck/steel", { 0.36f, 0.37f, 0.39f }, 0.55f);
    surface(MaterialId::BridgeDeck, kBridgeDeckWood,  "BridgeDeck/wood",  { 0.38f, 0.29f, 0.19f }, 0.86f);
    surface(MaterialId::BridgeDeck, kBridgeDeckStone, "BridgeDeck/stone", { 0.46f, 0.44f, 0.41f }, 0.84f);

    surface(MaterialId::Parapet, kParapetSteel,    "Parapet/steel",    { 0.34f, 0.35f, 0.37f }, 0.50f);
    surface(MaterialId::Parapet, kParapetStone,    "Parapet/stone",    { 0.48f, 0.46f, 0.43f }, 0.84f);
    surface(MaterialId::Parapet, kParapetConcrete, "Parapet/concrete", { 0.55f, 0.54f, 0.52f }, 0.80f);

    // Buildings. The geometry side maps building:material and roof:material here;
    // nothing emits these slots yet, but the vocabulary has to exist before it can.
    surface(MaterialId::Wall, kWallBrick,    "Wall/brick",    { 0.42f, 0.24f, 0.18f }, 0.86f);
    surface(MaterialId::Wall, kWallStone,    "Wall/stone",    { 0.48f, 0.46f, 0.42f }, 0.84f);
    surface(MaterialId::Wall, kWallConcrete, "Wall/concrete", { 0.55f, 0.54f, 0.52f }, 0.80f);
    surface(MaterialId::Wall, kWallRender,   "Wall/render",   { 0.72f, 0.70f, 0.66f }, 0.82f);
    surface(MaterialId::Wall, kWallGlass,    "Wall/glass",    { 0.22f, 0.26f, 0.30f }, 0.12f);
    surface(MaterialId::Wall, kWallMetal,    "Wall/metal",    { 0.40f, 0.41f, 0.43f }, 0.45f);
    surface(MaterialId::Wall, kWallWood,     "Wall/wood",     { 0.36f, 0.27f, 0.18f }, 0.88f);

    surface(MaterialId::Roof, kRoofTile,     "Roof/tile",     { 0.40f, 0.22f, 0.16f }, 0.88f);
    surface(MaterialId::Roof, kRoofSlate,    "Roof/slate",    { 0.20f, 0.21f, 0.23f }, 0.72f);
    surface(MaterialId::Roof, kRoofMetal,    "Roof/metal",    { 0.34f, 0.36f, 0.38f }, 0.48f);
    surface(MaterialId::Roof, kRoofMembrane, "Roof/membrane", { 0.24f, 0.24f, 0.25f }, 0.90f);
    surface(MaterialId::Roof, kRoofConcrete, "Roof/concrete", { 0.52f, 0.51f, 0.49f }, 0.84f);
    surface(MaterialId::Roof, kRoofThatch,   "Roof/thatch",   { 0.52f, 0.42f, 0.24f }, 0.94f);
    surface(MaterialId::Roof, kRoofGlass,    "Roof/glass",    { 0.24f, 0.28f, 0.32f }, 0.12f);

    // Markings cannot go through surface(): paint is a DECAL, so it needs the alpha
    // blend and the depth bias that put it above the carriageway without z-fighting.
    // surface() authors opaque surfaces and would drop both, and the result would be
    // opaque white quads hovering over the road.
    {
        const auto marking = [&](uint16_t variant, const char* name, glm::vec3 rgb) {
            MaterialDef def = resolve(MaterialKey{ MaterialId::Markings, 0 });
            def.name = name;
            def.base_color = glm::vec4{ rgb, 1.0f };
            set(MaterialKey{ MaterialId::Markings, variant }, std::move(def));
        };
        // Yellow paint: box junctions, bus lanes, no-waiting lines.
        marking(kMarkingsYellow, "Markings/yellow", { 0.86f, 0.68f, 0.10f });
        // Faded paint on a carriageway that resolved as worn; grey, not white.
        marking(kMarkingsWorn,   "Markings/worn",   { 0.72f, 0.71f, 0.68f });
    }
...
bool MaterialLibrary::install_procedural_textures(uint32_t texture_size, uint32_t seed) {
    ...
    attach(MaterialKey{ MaterialId::Asphalt, kAsphaltDefault },      asphalt,        { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Asphalt, kAsphaltSmooth },       asphalt_smooth, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Asphalt, kAsphaltWorn },         asphalt_worn,   { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Asphalt, kAsphaltCobble },  cobble,         { 2.0f, 2.0f });
    attach(MaterialKey{ MaterialId::Asphalt, kAsphaltSett },         sett,           { 2.0f, 2.0f });
    attach(MaterialKey{ MaterialId::Asphalt, kAsphaltPaving }, paving_block,   { 2.0f, 2.0f });

    // Concrete tiles at 4 m and the concrete generator is authored at 4 m.
    attach(MaterialKey{ MaterialId::Concrete, kConcreteDefault }, concrete, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Concrete, kConcreteSmooth },  concrete, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Concrete, kConcreteWorn },    concrete, { 1.0f, 1.0f });
    ...
    attach(MaterialKey{ MaterialId::Sidewalk, kSidewalkDefault },       paving_slab,    { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Sidewalk, kSidewalkAsphalt },        asphalt_smooth, { 0.25f, 0.25f });
    attach(MaterialKey{ MaterialId::Sidewalk, kSidewalkConcrete },          concrete,       { 0.5f, 0.5f });
    attach(MaterialKey{ MaterialId::Sidewalk, kSidewalkBrick },   cobble,         { 0.5f, 0.5f });
    attach(MaterialKey{ MaterialId::Sidewalk, kSidewalkBrick },          sett,           { 0.5f, 0.5f });
    attach(MaterialKey{ MaterialId::Sidewalk, kSidewalkPaved },  paving_block,   { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Sidewalk, kSidewalkTactile }, tactile,        { 1.0f, 1.0f });
    ...
    attach(MaterialKey{ MaterialId::Curb, kCurbDefault }, kerb, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Curb, kCurbGranite },  kerb, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Curb, kCurbAsphalt },    kerb, { 1.0f, 1.0f });

    attach(MaterialKey{ MaterialId::Gravel, kGravelDefault }, gravel, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Dirt,   kDirtDefault }, dirt,   { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Grass,  kGrassDefault }, grass,  { 1.0f, 1.0f });

    attach(MaterialKey{ MaterialId::BridgeDeck, kBridgeDeckDefault }, concrete, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::BridgeDeck, kBridgeDeckStone },  concrete, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::BridgeDeck, kBridgeDeckWood },    concrete, { 1.0f, 1.0f });
    attach(MaterialKey{ MaterialId::Parapet,    kParapetDefault }, concrete, { 1.0f, 1.0f });
